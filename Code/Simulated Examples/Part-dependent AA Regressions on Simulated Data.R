...
# Churn Data
source("Simulate Data - Churn Independence + Partial Dependence.R")
synthesized_data <- read_csv("part_dependent_aa_churn.csv")
...
final_results_partial <- bind_rows(
  ...
  mutate(data = "churn dependent on aa, aa independent")

rm(list = setdiff(ls(), c("final_results_partial", "final_results_full", "final_results_ind")))
